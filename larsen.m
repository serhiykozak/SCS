% TRY..CATCH added to original algorithm; "\" operator replaced with faster linsolve()
...
opts_utconj = struct('UT', true, 'RECT', true, 'TRANSA', true);
opts_ut = struct('UT', true, 'RECT', true);
...
while length(A) < maxVariables && ~stopCond && step < maxSteps
  ...
  if useGram
    try
        ...
        b_OLS = pinv(Gram(A,A))*(X(:,A)'*y); 
    end
  else
%     gR = gpuArray(R);
%     gX = gpuArray(X(:,A));
%     gy = gpuArray(y);
%     b_OLS = gather(gR\(gR'\(gX'*gy))); % same as X(:,A)\y, but faster
%     b_OLS = R\(R'\(X(:,A)'*y)); % same as X(:,A)\y, but faster
      b_OLS = linsolve(R, linsolve(R, (X(:,A)'*y), opts_utconj), opts_ut);
        % If A has the properties in opts, linsolve is faster than mldivide, 
        % because linsolve does not perform any tests to verify that A has the specified properties.
  end
  d = X(:,A)*b_OLS - mu;
  ...
  if isempty(I)
    ...
    if isempty(temp)
      warning('SpaSM:larsen', 'Could not find a positive direction towards the next event.');
      temp=0;
    end
    gamma = temp(1);
  end
